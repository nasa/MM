...
/SCX_CPU1_EVS_EnaAppEVTType Application=MMAppName DEBUG
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {MMAppName}, MM_NOOP_INF_EID, "INFO", 1

ut_sendcmd "SCX_CPU1_MM_NOOP"
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1000;1009) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements MM_1000, "P"
  ut_setrequirements MM_1009, "P"

  ;; Verify the HK fields
  if (SCX_CPU1_MM_LastActn = MM_NOOP) AND (SCX_CPU1_MM_MemType = 0) AND ;;
     (SCX_CPU1_MM_Address = 0) AND (SCX_CPU1_MM_DataValue = 0) AND ;;
     (SCX_CPU1_MM_BytesProc = 0) AND (SCX_CPU1_MM_LastFile = "") THEN
  ...
  write "<!> Failed (1000;1009) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",MM_NOOP_INF_EID, "."
  ut_setrequirements MM_1000, "F"
  ut_setrequirements MM_1009, "F"
endif
...
local errcnt = SCX_CPU1_MM_CMDEC + 1
rawcmd = ""
...
ut_tlmwait SCX_CPU1_MM_CMDEC, {errcnt}
if (UT_TW_Status = UT_Success) then
...
if (SCX_CPU1_MM_CMDPC = 0) then
  ;; Send a NOOP command
  /SCX_CPU1_MM_NOOP
  wait 5
endif

if (SCX_CPU1_MM_CMDEC = 0) then
  /raw {rawcmd}
  wait 5
endif

;; Setup for the expected event
ut_setupevents "SCX", "CPU1", {MMAppName}, MM_RESET_DBG_EID, "DEBUG", 1

/SCX_CPU1_MM_RESETCTRS
wait 5

ut_tlmwait SCX_CPU1_MM_CMDPC, 0
if (UT_TW_Status = UT_Success) then
...
ut_tlmwait SCX_CPU1_MM_CMDEC, 0
if (UT_TW_Status = UT_Success) then
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  write "<*> Passed (1001) - Event message ",SCX_CPU1_find_event[1].eventid, " received"
  ut_setrequirements MM_1001, "P"
  ;; Verify the HK fields
  if (SCX_CPU1_MM_LastActn = MM_RESET) AND (SCX_CPU1_MM_MemType = 0) AND ;;
     (SCX_CPU1_MM_Address = 0) AND (SCX_CPU1_MM_DataValue = 0) AND ;;
     (SCX_CPU1_MM_BytesProc = 0) AND (SCX_CPU1_MM_LastFile = "") THEN
    write "<*> Passed (8000) - Last Command HK items as expected"
    ut_setrequirements MM_8000, "P"
  ...
  write "<!> Failed (1001) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",MM_RESET_DBG_EID, "."
  ut_setrequirements MM_1001, "F"
endif
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {MMAppName}, MM_CC1_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_MM_CMDEC + 1

;;; CPU1 is the default
rawcmd = "1888c0000001aa00"
if ("CPU1" = "CPU2") then
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1010) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",MM_CC1_ERR_EID, "."
  ut_setrequirements MM_1010, "F"
endif
...
/SCX_CPU1_MM_SymTbl2File FileName="/ram/mm_symbol_tbl.dat"
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) then
  ;; Verify the HK fields
  if (SCX_CPU1_MM_LastActn = MM_SYMTBL_SAVE) AND ;;
     (SCX_CPU1_MM_MemType = 0) AND ;;
     (SCX_CPU1_MM_Address = 0) AND (SCX_CPU1_MM_DataValue = 0) AND ;;
     (SCX_CPU1_MM_BytesProc = 0) AND ;;
     (SCX_CPU1_MM_LastFile = "/ram/mm_symbol_tbl.dat") THEN
    write "<*> Passed (8000) - Last Command HK items as expected"
    ut_setrequirements MM_8000, "P"
  else
    write "<!> Failed (8000) - Last COmmand HK NOT correct."
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_mm_cmds                                    "
write ";*********************************************************************"
ENDPROC
